...
export default function Header() {
    ...
    const bg = useTransform(rotate, r => `conic-gradient(from ${r}deg,#1cca50 0deg,#1cca50 60deg,transparent 60deg)`)
    ...
    return (
        <header className="relative m-4 rounded-[40px]">
            ...
            />
            <div className={`relative ${isDark ? 'bg-[#101219]' : 'bg-[#0070f3]'} rounded-[40px] overflow-hidden`}>
                <nav className="flex justify-between items-center p-7">
                    <ul className="flex gap-8">
                        <li>{renderLink('/', 'Home')}</li>
                        {isAuthenticated && hasOrders && <li>{renderLink('/orders', 'My orders')}</li>}
                    </ul>
                    <div className="flex items-center gap-4">
                        {!isAuthenticated && !['/auth', '/register'].includes(pathname) && (
                            <Link
                                href="/auth"
                        ...
                        {isAuthenticated && (
                            <>
                                <Link href="/cart" className="text-2xl text-white hover:text-gray-300">
                                    🛒
                                </Link>
                                <button
