...
const Auth: React.FC = () => {
    const { register, handleSubmit, formState: { errors }, setError, clearErrors } = useForm<AuthFormData>();
    ...
    const handleLogin = async (data: AuthFormData) => {
        ...
        const res = await fetch("/api/login", {
            ...
            credentials: "include",
            body: JSON.stringify({ email: data.email, password: data.password, token: captchaToken }),
        });
            ...
            <div className="relative p-[3px] rounded-[40px] w-full max-w-md">
                ...
                <div className="relative rounded-[40px] overflow-hidden p-12 shadow-lg bg-[#101219]">
                    ...
                    <div className="mt-6 text-center">
                        ...
                        <Link href="/register" className="cursor-pointer hover:underline font-semibold text-base">
                            {renderAnimatedText("Register here")}
                        </Link>
                    </div>
                </div>
            </div>
